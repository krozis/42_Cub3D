...
/*   Updated: 2023/01/27 16:37:24 by dcyprien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	drawline(t_ptr *ptr, int x, int drawstart, int drawend)
{
    ...
    if (ptr->ray->side == 0)
		wallX = ptr->player->posY + ptr->ray->perpWallDist * ptr->ray->raydirX;
    else
		wallX = ptr->player->posX + ptr->ray->perpWallDist * ptr->ray->raydirY;
    wallX -= floor((wallX));
    ...
    while (i <= drawend)
    {
        ...
        texPos += step;
        int colour = convert_colors(ptr->dply.textures[0].data[4 * (64 * texY + texX)], ptr->dply.textures[0].data[4 * (64 * texY + texX) + 1], ptr->dply.textures[0].data[4 * (64 * texY + texX) + 2]);
        //make color darker for y-sides: R, G and B byte each divided through two with a "shift" and an "and"
        if(ptr->ray->side == 0)
			colour = (colour >> 1) & 8355711;
		ptr->dply.screen->data[4 * (i * WIN_WIDTH + x)] = (colour / RGB_RED);
		ptr->dply.screen->data[4 * (i * WIN_WIDTH + x) + 1] = (colour % RGB_RED) / RGB_GREEN;
		ptr->dply.screen->data[4 * (i * WIN_WIDTH + x) + 2] = (colour % RGB_RED) % RGB_GREEN;
		i++;
	}
